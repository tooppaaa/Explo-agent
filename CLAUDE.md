...
4. **Auth par requête** : un Bearer reçu sur `/chat`, `/confirm` ou `/mcp` surcharge le credential de service (`tokenOverrides`, jamais visible du sandbox). En prod (`AUTH_MODE=required`, défaut si `NODE_ENV=production`) : 401 sans Bearer — sinon le credential de service servirait de fallback à quiconque atteint l'endpoint. Le widget passe le token via `initAgent({ auth: { token } })`.
5. **Validation systématique** : args validés (Zod) côté hôte avant tout appel HTTP.
6. **Mutations** : mode `intent` par défaut — une op mutante est bloquée par le bridge ; le moteur stocke l'**intent** (`opName` + `args`, TTL 10 min) et `POST /confirm` ne rejoue QUE cet appel (jamais le code sandbox complet — TOCTOU et mutations cachées).
...
| `packages/sandbox` | `DenoWorkerExecutor` (`permissions:none`, pool de process réutilisables — Worker frais par exec) + `HttpHostBridge` (HTTP + credential) |
...
export AUTH_MODE=optional               # "required" en prod : 401 sans Bearer sur /chat,/confirm,/mcp
export RATE_LIMIT_MAX=30                # req/IP/fenêtre (0 = off) ; RATE_LIMIT_WINDOW_MS=60000
```
...
`tests/search.test.ts` (§10.1-2), `tests/sandbox.test.ts` (§10.3, §10.7 + pool),
`tests/sandbox-isolation.test.ts` (§10.4, §10.5), `tests/engine.test.ts` (§10.1, §10.6, §10.8),
`tests/mcp-server.test.ts` (interface MCP), `tests/chat-backend.test.ts` (boucle complète),
`tests/chat-security.test.ts` (auth + rate limiting), `tests/catalogue.test.ts`, `tests/mock-api.test.ts`.
